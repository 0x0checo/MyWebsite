    ...
    <section id="home" className="py-20">
      <div className="mx-auto w-full max-w-5xl px-6">
        <div className="max-w-3xl">
          ...
          <p className="mt-4 text-lg text-neutral-200">{site.tagline}</p>

          <ul className="mt-6 flex flex-wrap gap-2">
            {site.highlights.map((h) => (
              <li
                key={h}
                className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-sm text-neutral-200 backdrop-blur
...
transition-all duration-200 hover:-translate-y-0.5 hover:opacity-95"
            ...
            {/* Glass buttons */}
            <a
              className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-4 py-2 text-sm text-neutral-200 backdrop-blur
transition-all duration-200
